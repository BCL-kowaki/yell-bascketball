...
export default function AdminBannersPage() {
  ...
  async function loadBanners() {
    try {
      const data = await getSiteBanners()
      setBanners(data)
    } catch (error) {
      console.error("運営バナー読み込みエラー:", error)
    }
  }
  ...
  async function handleSave() {
    ...
    setSaveMessage(null)
    try {
      await updateSiteBanners(banners, currentEmail)
      setSaveMessage("保存しました")
      setTimeout(() => setSaveMessage(null), 3000)
    } catch (error) {
      console.error("運営バナー保存エラー:", error)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-bold">運営バナー管理</h1>
            </div>
          </div>
          <Button
            onClick={handleSave}
      ...
      <div className="max-w-4xl mx-auto px-2 py-6 space-y-6">
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="pt-4">
            <p className="text-sm text-gray-600">
              運営バナーはユーザーのプロフィールページのサイドバーに表示されます。
              YeLL公式からのお知らせや運営情報のバナーを設定できます。最大5つまで登録可能です。
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader>
            <h2 className="font-semibold">バナー編集</h2>
          </CardHeader>
          <CardContent>
            <SponsorBannerEditor
              sponsors={banners}
