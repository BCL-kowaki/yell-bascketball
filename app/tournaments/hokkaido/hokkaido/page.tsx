...
// 北海道の大会データ
const hokkaidoTournaments = [
  {
    id: "hokkaido-spring-championship",
    title: "北海道春季バスケットボール選手権",
    organizer: "北海道バスケットボール協会",
  ...
  {
    id: "asahikawa-women-league",
    title: "旭川女子バスケットリーグ",
    organizer: "旭川女子バスケット連盟",
  ...
  {
    id: "corporate-league",
    title: "北海道企業対抗バスケットリーグ",
    organizer: "北海道企業スポーツ連盟",
...
export default function HokkaidoTournamentsPage() {
  ...
  const filteredTournaments = hokkaidoTournaments.filter(tournament => {
    ...
    <Layout isLoggedIn={true} currentUser={{ name: "ユーザー" }}>
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex items-center gap-2 mb-6 text-sm">
          ...
          <Link href="/tournaments/hokkaido" className="text-gray-500 hover:text-gray-700">
            北海道エリア
          </Link>
          <ChevronRight className="w-4 h-4 text-gray-400" />
          <span className="text-red-600 font-medium">北海道</span>
        </div>
        {/* Header */}
        {/* Statistics */}
        {/* Search and Filter */}
          ...
          <TabsContent value={selectedTab} className="mt-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredTournaments.map((tournament) => (
                <Link key={tournament.id} href={`/tournaments/kanto/hokkaido/${tournament.id}`}>
