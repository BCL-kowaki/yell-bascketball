...
export default function CreateTeamPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    console.log("Team Creation Data:", formData)
    toast({
      title: "チーム作成申請を送信しました",
      description: "運営者の承認をお待ちください。承認されるとチームページが公開されます。",
    })
  }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input id="name" value={formData.name} onChange={handleInputChange} required className="bg-white" />
                </div>
                 <div className="space-y-2">
                  <Label htmlFor="shortName">チーム略称</Label>
                  <Input id="shortName" value={formData.shortName} onChange={handleInputChange} className="bg-white" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label>ロゴ画像</Label>
                   <div className="w-32 h-32 border-2 border-dashed rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-50" onClick={() => document.getElementById('logo')?.click()}>
                ...
                <div className="space-y-2">
                   <Label>カバー画像</Label>
                   <div className="w-full h-32 border-2 border-dashed rounded-lg flex items-center justify-center cursor-pointer hover:bg-gray-50" onClick={() => document.getElementById('coverImage')?.click()}>
                   ...
                   <Input id="coverImage" type="file" accept="image/*" onChange={(e) => handleFileChange(e, "coverImage")} className="hidden" />
                </div>
              </div>
               ...
               <div className="space-y-2">
                <Label>カテゴリ</Label>
                <RadioGroup value={formData.category} onValueChange={(v) => handleSelectChange("category", v)} className="flex items-center gap-6">
                  <div className="flex items-center space-x-2"><RadioGroupItem value="U12" id="u12" /><Label htmlFor="u12">U12</Label></div>
                  <div className="flex items-center space-x-2"><RadioGroupItem value="U15" id="u15" /><Label htmlFor="u15">U15</Label></div>
                  <div className="flex items-center space-x-2"><RadioGroupItem value="U18" id="u18" /><Label htmlFor="u18">U18</Label></div>
                </RadioGroup>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input id="founded" value={formData.founded} onChange={handleInputChange} className="bg-white" />
                </div>
                 <div className="space-y-2">
                  <Label htmlFor="headcount">人数</Label>
                  <Input id="headcount" type="number" value={formData.headcount} onChange={handleInputChange} className="bg-white" />
                </div>
              </div>

              <div className="space-y-2">
                  <Label htmlFor="website">ウェブサイト・SNSリンク</Label>
                  <Input id="website" type="url" value={formData.website} onChange={handleInputChange} className="bg-white" />
              </div>
